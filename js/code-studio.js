...
function openDownloadAppModal() {
    saveCodeStudioCurrentCode();
    var old = document.getElementById('downloadAppModal');
    if (old) old.remove();
    
    var projectName = document.getElementById('codeStudioProjectName');
    var defaultName = projectName ? projectName.value || 'Meu App' : 'Meu App';
    var safeName = defaultName.toLowerCase().replace(/[^a-z0-9]/gi, '');
    ...
    modal.innerHTML = `
        <div class="cs-app-modal">
            <div class="cs-app-title">Criar APK</div>
            <div class="cs-app-subtitle">Transforme seu projeto em um app Android</div>
            <div class="cs-app-form">
                <label>Nome do App</label>
                <input type="text" id="appNameInput" value="${defaultName}" placeholder="Digite o nome do app">
                
                <label>Package ID</label>
                <input type="text" id="appPackageInput" value="com.codestudio.${safeName}" placeholder="com.exemplo.app">
                
                <label>Ícone do App</label>
                <div class="cs-app-icon-area">
                    <div class="cs-app-icon-preview" id="appIconPreview">
                        <i class="fas fa-mobile-alt"></i>
                    </div>
                    <button class="cs-app-icon-btn" onclick="selectAppIcon()">
                        <i class="fas fa-image"></i> Escolher Imagem
                    </button>
                </div>
                <input type="file" id="appIconInput" accept="image/*" style="display:none" onchange="handleAppIconSelect(this)">
                
                <div id="apkBuildStatus" style="display:none;margin-top:16px;padding:16px;background:rgba(16,185,129,0.1);border-radius:12px;border:1px solid rgba(16,185,129,0.2);">
                    <div style="display:flex;align-items:center;gap:12px;">
                        <div class="apk-spinner"></div>
                        <span id="apkStatusText" style="color:#10b981;font-size:13px;font-weight:500;">Preparando...</span>
                    </div>
                    <div id="apkProgress" style="margin-top:12px;height:4px;background:rgba(16,185,129,0.2);border-radius:4px;overflow:hidden;">
                        <div id="apkProgressBar" style="width:0%;height:100%;background:linear-gradient(90deg,#10b981,#059669);transition:width 0.3s;"></div>
                    </div>
                </div>
            </div>
            <div class="cs-confirm-btns">
                <button class="cs-confirm-cancel" onclick="this.closest('.cs-confirm-overlay').remove()">Cancelar</button>
                <button class="cs-confirm-ok cs-app-create" id="buildApkBtn" onclick="buildRealApk()">Gerar APK</button>
            </div>
        </div>`;
    document.body.appendChild(modal);
    modal.onclick = function(e) { if (e.target === modal) modal.remove(); };
    downloadAppState.iconDataUrl = null;
    
    // Atualizar package quando nome muda
    document.getElementById('appNameInput').oninput = function() {
        var name = this.value.toLowerCase().replace(/[^a-z0-9]/gi, '');
        document.getElementById('appPackageInput').value = 'com.codestudio.' + (name || 'app');
    };
}
