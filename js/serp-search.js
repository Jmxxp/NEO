...
async function serpSearchWithKey(query, apiKey, numResults) {
    ...
    // Usar proxy CORS se não estiver no Cordova (ou se for versão web)
    const isCordova = !window.IS_WEB_VERSION && !!(window.cordova || window.Cordova || document.URL.indexOf('http://') === -1);
