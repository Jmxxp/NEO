...
// ===== HANDLER ESPECIAL PARA iOS SAFARI - BOTÃO DE CHAMADA =====
// iOS Safari tem problemas com eventos click sintéticos, então tratamos diretamente
(function() {
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || 
                  (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
    
    // Aguardar DOM e scripts carregados
    function initSendCallBtn() {
        const sendCallBtn = document.getElementById('sendCallBtn');
        if (!sendCallBtn) {
            console.warn('⚠️ sendCallBtn não encontrado');
            return;
        }
        
        console.log('📱 [iOS FIX] Configurando sendCallBtn para', isIOS ? 'iOS' : 'outro dispositivo');
        
        // === MÉTODO 1: Captura direta de toque ===
        ...
        sendCallBtn.addEventListener('touchstart', function(e) {
            ...
            // NÃO usar preventDefault aqui para permitir feedback visual
        }, { passive: true });
        ...
        sendCallBtn.addEventListener('touchend', function(e) {
            ...
            // Tap válido: < 500ms e < 30px de movimento
            if (touchDuration < 500 && moveX < 30 && moveY < 30) {
                console.log('👆 [iOS FIX] Touch válido detectado');
                e.preventDefault();
                e.stopPropagation();
                e.stopImmediatePropagation();
                
                // Executar com pequeno delay para iOS Safari
                setTimeout(function() {
                    executeSendCallAction();
                }, 10);
            }
        }, { passive: false });
        
        // === MÉTODO 2: Click handler como fallback ===
        sendCallBtn.addEventListener('click', function(e) {
            // Só processar se não foi um touch (para evitar duplo disparo)
            if (touchActive) return;
            console.log('🖱️ [iOS FIX] Click detectado');
            e.preventDefault();
            e.stopPropagation();
            executeSendCallAction();
        });
        
        console.log('✅ [iOS FIX] Handlers configurados no sendCallBtn');
    }
    
    // Função que executa a ação do botão
    function executeSendCallAction() {
        const sendCallBtn = document.getElementById('sendCallBtn');
        const input = document.getElementById('user-input');
        
        console.log('🔥 [iOS FIX] executeSendCallAction chamado');
        
        // Verificar se está no modo stop
        if (sendCallBtn && sendCallBtn.classList.contains('stop-mode')) {
            console.log('⏹️ [iOS FIX] Modo stop - parando geração...');
        ...
        console.log('📊 [iOS FIX] Estado:', { hasText, hasAttachments });
        
        if (!hasText && !hasAttachments) {
            // Modo chamada de voz
            console.log('📞 [iOS FIX] Iniciando chamada de voz...');
            
            // Tentar todas as formas de chamar startVoiceCall
            if (typeof window.startVoiceCall === 'function') {
                console.log('📞 [iOS FIX] Chamando window.startVoiceCall');
                window.startVoiceCall();
            } else if (typeof startVoiceCall === 'function') {
                console.log('📞 [iOS FIX] Chamando startVoiceCall direto');
                startVoiceCall();
            } else {
                console.error('❌ [iOS FIX] startVoiceCall NÃO disponível!');
                // Debug: mostrar o que está disponível
                console.log('🔍 window.startVoiceCall:', typeof window.startVoiceCall);
                console.log('🔍 startVoiceCall global:', typeof startVoiceCall);
            ...
            console.log('📤 [iOS FIX] Enviando mensagem...');
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initSendCallBtn);
    } else {
        initSendCallBtn();
    }
    
    // Expor função globalmente para debug
    window._executeSendCallAction = executeSendCallAction;
})();
