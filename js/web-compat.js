...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // When iOS keyboard opens, it pushes the webview up.
        // We prevent that by making html position:fixed (in CSS)
        // and resizing via --app-height to fit the visual viewport.
        ...
        if (window.visualViewport) {
            ...
            function handleViewportResize() {
                ...
                if (!topBarEl) topBarEl = document.querySelector('.top-bar');

                var vp = window.visualViewport;
                var keyboardH = window.innerHeight - vp.height;
                var kbOpen = keyboardH > 100;
                
                // Resize the entire page to the visual viewport height
                document.documentElement.style.setProperty('--app-height', vp.height + 'px');
                
                if (kbOpen) {
                    // Keyboard open: shrink padding so content uses available space
                    if (appEl) {
                        appEl.style.paddingTop = '70px';
                        appEl.style.paddingBottom = '70px';
                    }
                    // Input bar: position within the shrunken container
                    // Since html is now vp.height, bottom:10px puts it at bottom of visible area
                    if (inputBar) inputBar.style.bottom = '10px';
                    // Top bar: bring closer to top
                    if (topBarEl) topBarEl.style.top = '2px';
                    
                    // Scroll messages to bottom on first open
                    if (!lastKbOpen) {
                        setTimeout(function() {
