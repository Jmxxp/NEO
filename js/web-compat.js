...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // When keyboard opens: shrink .app to fit visible area above keyboard
        // Uses height + transform to squish the layout into the visible portion
        if (window.visualViewport) {
            ...
            function handleViewportResize() {
                ...
                if (!inputBar || !appEl) return;

                var vp = window.visualViewport;
                var kbHeight = window.innerHeight - vp.height;
                var kbOpen = kbHeight > 100;
                
                if (kbOpen) {
                    var visibleHeight = vp.height;
                    
                    // Squish the entire app into the visible portion
                    appEl.style.height = visibleHeight + 'px';
                    appEl.style.maxHeight = visibleHeight + 'px';
                    appEl.style.overflow = 'hidden';
                    appEl.style.paddingTop = '65px';
                    appEl.style.paddingBottom = '70px';
                    
                    // Position input bar at the bottom of the visible area
                    inputBar.style.position = 'fixed';
                    inputBar.style.bottom = (kbHeight + 5) + 'px';
                    
                    // Top bar stays at top of visible area
                    if (topBar) {
                        topBar.style.top = '2px';
                    }
