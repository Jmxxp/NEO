...
function updateSpeedButtonUI() {
    const speedBtn = document.getElementById('voiceCallSpeedBtn');
    if (speedBtn) {
        speedBtn.textContent = voiceCallSpeechRate + 'x';
    }
}
...
function getVoiceProvider() {
    ...
    return localStorage.getItem('neoVoiceProvider') || 'elevenlabs';
}
...
async function processVoiceCallMessage(userText) {
    ...
    try {
        ...
        if (voiceCallActive) {
            // Tentar dar uma resposta genérica ao invés de pedir para repetir
            const respostaFallback = gerarRespostaFallback(userText);
            // O speakInCall já cuida de reiniciar o reconhecimento após falar
            speakInCall(respostaFallback);
        } else {
...
// Obtém resposta da IA - VERSÃO RÁPIDA
async function getVoiceCallResponse(userText) {
    ...
    // Pegar a chave da API DeepSeek (usando novo sistema de múltiplas chaves)
    let deepseekKey = '';
    try {
        if (typeof getAllApiKeys === 'function') {
            const keys = getAllApiKeys('deepseek');
            deepseekKey = keys[0] || '';
        }
    } catch (e) {
        console.error('❌ Erro ao pegar chave:', e);
    }

    console.log('🔑 [VoiceCall] Chave encontrada:', deepseekKey ? 'SIM (' + deepseekKey.substring(0, 5) + '...)' : 'NÃO');

    if (!deepseekKey) {
        console.error('❌ [VoiceCall] Chave DeepSeek não configurada!');
        throw new Error('Chave DeepSeek não configurada. Vá em Configurações e adicione sua chave.');
    }
    ...
    // System prompt otimizado para voz - DeepSeek
    const systemPrompt = `Você é Neo, um assistente de voz inteligente conversando em português brasileiro.
...
${conversationContext}`;
    ...
    try {
        const apiUrl = typeof getApiUrl === 'function'
            ? getApiUrl("https://api.deepseek.com/chat/completions")
        ...
        console.log('✅ [VoiceCall] Resposta DeepSeek:', resposta);
        return resposta;

    } catch (error) {
        clearTimeout(timeoutId);
        console.error('❌ [VoiceCall] Erro DeepSeek:', error);
        if (error.name === 'AbortError') {
            console.log('⏱️ Timeout na chamada DeepSeek');
        }
        throw error;
    }
}
...
function speakInCall(text, onEnd = null) {
    ...
    // VOZ NATIVA - Usa Web Speech API do navegador/sistema
    if (provider === 'native') {
    ...
    // GOOGLE TTS (padrão/fallback)
    console.log('✅ Usando Google TTS');
    speakWithGoogleTTS(text, onEndWrapper);
}
...
function updateCallUI(state) {
    ...
    switch (state) {
        case 'listening':
            ...
            break;
        case 'speaking':
            status.textContent = 'Neo está falando... (toque para parar)';
